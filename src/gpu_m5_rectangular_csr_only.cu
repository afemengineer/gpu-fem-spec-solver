...
void check_cuda_rect(cudaError_t status, const char* what) {
...
double median_rect(std::vector<double> values) {
...
GpuM5RectangularTransferResult benchmark_m5_rectangular_csr(
    ...
    if (repeats <= 0) throw std::invalid_argument("M5 rectangular transfer repeats must be positive");
    ...
    for (const auto c : column_indices) {
        if (c >= cols) throw std::invalid_argument("M5 rectangular transfer column out of range");
    }
    ...
    try {
        check_cuda_rect(cudaMalloc(reinterpret_cast<void**>(&d_rows), row_bytes), "cudaMalloc(M5 transfer rows)");
        check_cuda_rect(cudaMalloc(reinterpret_cast<void**>(&d_cols), col_bytes), "cudaMalloc(M5 transfer columns)");
        check_cuda_rect(cudaMalloc(reinterpret_cast<void**>(&d_values), val_bytes), "cudaMalloc(M5 transfer values)");
        check_cuda_rect(cudaMalloc(reinterpret_cast<void**>(&d_x), x_bytes), "cudaMalloc(M5 transfer x)");
        check_cuda_rect(cudaMalloc(reinterpret_cast<void**>(&d_y), y_bytes), "cudaMalloc(M5 transfer y)");
        check_cuda_rect(cudaMemcpy(d_rows, row_offsets.data(), row_bytes, cudaMemcpyHostToDevice), "cudaMemcpy(M5 transfer rows H2D)");
        check_cuda_rect(cudaMemcpy(d_cols, column_indices.data(), col_bytes, cudaMemcpyHostToDevice), "cudaMemcpy(M5 transfer columns H2D)");
        check_cuda_rect(cudaMemcpy(d_values, values.data(), val_bytes, cudaMemcpyHostToDevice), "cudaMemcpy(M5 transfer values H2D)");
        check_cuda_rect(cudaMemcpy(d_x, x.data(), x_bytes, cudaMemcpyHostToDevice), "cudaMemcpy(M5 transfer x H2D)");

        constexpr unsigned int threads = 256U;
        constexpr unsigned int warps_per_block = threads / 32U;
        const unsigned int blocks = static_cast<unsigned int>((rows + warps_per_block - 1U) / warps_per_block);

        m5_rectangular_csr_warp_kernel<<<blocks, threads>>>(
            static_cast<std::uint32_t>(rows), d_rows, d_cols, d_values, d_x, d_y);
        check_cuda_rect(cudaGetLastError(), "M5 transfer warmup launch");
        check_cuda_rect(cudaDeviceSynchronize(), "M5 transfer warmup sync");

        check_cuda_rect(cudaEventCreate(&start), "cudaEventCreate(M5 transfer start)");
        check_cuda_rect(cudaEventCreate(&stop), "cudaEventCreate(M5 transfer stop)");
        std::vector<double> samples;
        samples.reserve(static_cast<std::size_t>(repeats));
        for (int repeat = 0; repeat < repeats; ++repeat) {
            check_cuda_rect(cudaEventRecord(start), "cudaEventRecord(M5 transfer start)");
            m5_rectangular_csr_warp_kernel<<<blocks, threads>>>(
                static_cast<std::uint32_t>(rows), d_rows, d_cols, d_values, d_x, d_y);
            check_cuda_rect(cudaGetLastError(), "M5 transfer launch");
            check_cuda_rect(cudaEventRecord(stop), "cudaEventRecord(M5 transfer stop)");
            check_cuda_rect(cudaEventSynchronize(stop), "cudaEventSynchronize(M5 transfer)");
            float ms = 0.0f;
            check_cuda_rect(cudaEventElapsedTime(&ms, start, stop), "cudaEventElapsedTime(M5 transfer)");
        ...
        check_cuda_rect(cudaMemcpy(result.y.data(), d_y, y_bytes, cudaMemcpyDeviceToHost), "cudaMemcpy(M5 transfer y D2H)");
        result.timing.median_ms = median_rect(samples);
