...
// compare dense FP32 cuBLAS SGEMV, structural scalar CSR and natural 6x6
// block-CSR for the actual A2 produced at an arbitrary thin-plate refinement.
...
Block6Payload compress_block6(const m5_l2_setup::DenseA2& a2, std::size_t block_rows) {
    if (a2.n != block_rows * 6U ||
        block_rows > static_cast<std::size_t>(std::numeric_limits<std::uint32_t>::max())) {
        throw std::runtime_error("A2 is not a uniform rank-six block layout");
    }
    Block6Payload out;
    out.row_offsets.resize(block_rows + 1U, 0U);
    for (std::size_t br = 0U; br < block_rows; ++br) {
        out.row_offsets[br] = static_cast<std::uint32_t>(out.column_indices.size());
        for (std::size_t bc = 0U; bc < block_rows; ++bc) {
            ...
            if (!nonzero) continue;
            if (out.column_indices.size() >=
                static_cast<std::size_t>(std::numeric_limits<std::uint32_t>::max())) {
                throw std::runtime_error("A2 block6 nnz exceeds uint32 range");
            }
            out.column_indices.push_back(static_cast<std::uint32_t>(bc));
    ...
    out.row_offsets[block_rows] = static_cast<std::uint32_t>(out.column_indices.size());
    return out;
}
...
int main(int argc, char** argv) {
    try {
        ...
        const std::size_t l2_nodes = hierarchy.transfer1.aggregates.size();
        ...
        const auto block6 = compress_block6(a2, l2_nodes);
        if (values.size() != block6.column_indices.size() * 36U) {
        ...
        const auto gpu_block6 = gfss::benchmark_m5_l2_block6_csr(
            block6.row_offsets, block6.column_indices, block6.values, probe_f, repeats);

        const double csr_error = relative_error(gpu_csr.y, reference);
        const double dense_error = relative_error(gpu_dense.y, reference);
        const double block6_error = relative_error(gpu_block6.y, reference);
        ...
        const std::size_t block6_matrix_bytes = block6.row_offsets.size() * sizeof(std::uint32_t) +
            block6.column_indices.size() * sizeof(std::uint32_t) +
            block6.values.size() * sizeof(float);
        const double density = static_cast<double>(values.size()) /
            static_cast<double>(a2.n * a2.n);
        const double block_density = static_cast<double>(block6.column_indices.size()) /
            static_cast<double>(l2_nodes * l2_nodes);
        const bool oracle_accept = csr_error <= 1.0e-4 && dense_error <= 1.0e-4 &&
            block6_error <= 1.0e-4;

        const char* preferred = "dense_fp32_cublas_sgemv";
        double preferred_ms = gpu_dense.timing.median_ms;
        if (gpu_csr.timing.median_ms < preferred_ms) {
            preferred = "structural_scalar_CSR_fp32";
            preferred_ms = gpu_csr.timing.median_ms;
        }
        if (gpu_block6.timing.median_ms < preferred_ms) {
            preferred = "structural_block6_CSR_fp32";
            preferred_ms = gpu_block6.timing.median_ms;
        }

        std::cout << "GFSS M5 A2 runtime representation scaling probe\n"
                  << "problem=thin_plate mesh=" << nx << 'x' << ny << 'x' << nz
                  << " physical=1x1x0.125\n"
                  << "candidates=dense_fp32_cublas_sgemv,structural_scalar_CSR_fp32,structural_block6_CSR_fp32\n"
                  ...
                  << " gpu_block6_CSR_vs_FP64_dense_relative_error=" << block6_error
                  ...
                  << " A2_block_nnz=" << block6.column_indices.size()
                  << " A2_block_density=" << block_density << '\n'
                  << "A2_CSR_matrix_bytes=" << csr_matrix_bytes
                  << " A2_dense_matrix_bytes=" << dense_matrix_bytes
                  << " A2_block6_CSR_matrix_bytes=" << block6_matrix_bytes
                  << " CSR_over_dense_memory_ratio="
                  << static_cast<double>(csr_matrix_bytes) / static_cast<double>(dense_matrix_bytes)
                  << " block6_CSR_over_dense_memory_ratio="
                  << static_cast<double>(block6_matrix_bytes) / static_cast<double>(dense_matrix_bytes)
                  << " block6_CSR_over_scalar_CSR_memory_ratio="
                  << static_cast<double>(block6_matrix_bytes) / static_cast<double>(csr_matrix_bytes) << '\n'
                  ...
                  << " block6_CSR_median_ms=" << gpu_block6.timing.median_ms
                  << " block6_CSR_best_ms=" << gpu_block6.timing.best_ms << '\n'
                  << "dense_over_CSR_median_ratio="
                  << gpu_dense.timing.median_ms / gpu_csr.timing.median_ms
                  << " dense_over_block6_CSR_median_ratio="
                  << gpu_dense.timing.median_ms / gpu_block6.timing.median_ms
                  << " CSR_over_block6_CSR_median_ratio="
                  << gpu_csr.timing.median_ms / gpu_block6.timing.median_ms << '\n'
                  << "CSR_device_bytes_including_vectors=" << gpu_csr.device_bytes
                  << " dense_device_bytes_including_vectors=" << gpu_dense.device_bytes
                  << " block6_CSR_device_bytes_including_vectors=" << gpu_block6.device_bytes << '\n'
