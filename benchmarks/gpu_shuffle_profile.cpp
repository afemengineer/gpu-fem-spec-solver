...
int main(int argc, char** argv) {
    try {
        ...
        const auto result =
            gfss::apply_node_stencil_cuda_gold_shuffle(mesh, material, x, repeats, 4);
        ...
        std::cout << std::fixed << std::setprecision(3)
                  << "GFSS CUDA GoldShuffle profiler workload\n"
                  << "mesh=" << nx << 'x' << ny << 'x' << nz
                  << " dofs=" << mesh.dof_count() << '\n'
                  << "block=32x4x1\n"
        ...
        std::cerr << "error: " << e.what() << '\n'
                  << "usage: gfss_gpu_shuffle_profile [nx [ny nz [repeats]]]\n";
        return 1;
    }
}
