// M5 combined consistency test:
// theta=0.05 actual-A1 energy strength graph + localized exact actual-L2 block
// metric + localized exact recursive bottom, with all transfer restrictions
// equal to the exact transpose. CPU/FP64 numerical reference only.
#include "recursive_sa_local_l2_helpers.inc"
#include "recursive_sa_actual_a1_strength_local_helpers.inc"

namespace {

void run_strength_local_l2_consistency(std::size_t max_cycles,
    ...
    if (result.relative_residuals.size() > 2U) {
        ...
        if (count > 0U) post_transient_q = std::exp(log_sum / static_cast<double>(count));
    }
...
int main(int argc, char** argv) {
    try {
        ...
        run_strength_local_l2_consistency(max_cycles, target_nodes, min_nodes);
        return 0;
